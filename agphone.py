...
BANNER = f"""{G}{BOLD}
  ...
  ██║░░██║╚██████╔╝  ██║░░░░░██║░░██║╚█████╔╝██║░╚███║███████╗
  ╚═╝  ╚═╝░╚═════╝░  ╚═╝░░░░░╚═╝░░╚═╝░╚════╝░╚═╝░░╚══╝╚══════╝{R}
  {dim('Antigravity Phone Connect  ·  CLI v2.1')}
"""
...
def load_env():
    ...
    if os.path.exists(ENV_FILE):
        with open(ENV_FILE) as f:               # FIX: proper file close
            for line in f:
                line = line.strip()
    ...
    for cmd in TS_CANDIDATES:
        try:
            result = subprocess.run(
                [cmd, "ip", "-4"], capture_output=True, text=True, timeout=5)
...
def get_urls():
    ...
    proto = protocol()
    return proto, port, local_ip(), tailscale_ip()
...
def do_start():
    ...
    env_copy.update(env)
    log_handle = open(LOG_FILE, "a")             # kept open for child process lifetime
    proc = subprocess.Popen(
        ["node", "server.js"], cwd=DIR,
        stdout=log_handle, stderr=log_handle, env=env_copy)
    with open(PID_FILE, "w") as f:              # FIX: proper file close
    ...
    if ts_url:
        print_qr("Tailscale", ts_url, "🌐")

    notify("Antigravity Phone ✅", local_url + (f" | {ts_url}" if ts_url else ""))
...
def do_stop():
    ...
    if alive(pid):
        ...
        # Verify
        if alive(pid):
            print(f"  {r('✗')}  Could not stop server (PID {pid}).")
        else:
            print(f"  {g('✓')}  Stopped.")
...
def do_status():
    pid = read_pid()
    running = alive(pid)
    proto, port, lip, tip = get_urls()
    status = f"{g('● ONLINE')}" if running else f"{r('○ OFFLINE')}"
    stale = pid and not running

    print(f"\n{SEP}")
    print(f"  STATUS   {status}  {dim(f'PID {pid}') if running else ''}")
    if stale:
        print(f"  {y('⚠')}   Stale PID file — server may have crashed. Run [2] Stop to clean up.")
    print(SEP)
...
def do_qr():
    ...
    if tip:
        print_qr("Tailscale", f"{proto}://{tip}:{port}", "🌐")
    else:
...
def do_logs():
    if not os.path.exists(LOG_FILE):             # FIX: check file exists first
        print(f"\n  {dim('No log file yet. Start the server first.')}")
        return
    print(f"\n  {dim('Tailing')} {c(LOG_FILE)} {dim('— Ctrl+C to stop')}\n{SEP}")
    try:
        subprocess.run(["tail", "-n", "50", "-f", LOG_FILE])  # FIX: show last 50 lines first
...
def menu():
    ...
    while True:
        ...
        print(f"  {c('[r]')}  {BOLD}Restart{R} server")
